...
@router.get("/scene-state", response_model=DashboardSceneStateSnapshot, summary="Get backend-driven overview scene state")
def get_dashboard_scene_state() -> DashboardSceneStateSnapshot:
    if settings.DATA_SOURCE == "mock":
        mock_dashboard_service.tick()
    return state_store.get_scene_state()
