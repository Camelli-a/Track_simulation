...
def normalize_switch(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "active_to" not in normalized:
        if normalized.get("state") == "reverse" and normalized.get("reverse_to"):
            normalized["active_to"] = normalized["reverse_to"]
        elif normalized.get("state") == "normal" and normalized.get("normal_to"):
            normalized["active_to"] = normalized["normal_to"]
    return normalized
